...
const totalQuantity = cart.reduce((total, item) => total + item.quantity, 0);
  ...
  return (
    <>
      ...
      <header
        className={`relative sm:pl-10    fadded-borders bg-white  dark:bg-zinc-900 sm:pr-10 pt-[1%] pb-[1%] ${Class} navbar-header `}
        >
        <motion.div 
        initial={{opacity:0  }}
        ...
        <nav className="flex float-right">
          <ul className="navbar flex  sm:text-lg  sm:px-4 sm:gap-10 flex-row text-[#ae040f]  ">
            ...
            <li className="relative">
              <NavLink to={"/cart"} className=" cart-image-container invisible md:visible text-rose-800 w-8  h-8">
                <FaShoppingCart /> {totalQuantity > 0 && (
            ...
            <div>
              <button className="absolute  right-10" onClick={toggle}>
        ...
        <nav className="md:hidden fixed bottom-0 mt-[20%] left-0 w-[98vw] bg-white bottom-nav text-rose-800   shadow-lg">
